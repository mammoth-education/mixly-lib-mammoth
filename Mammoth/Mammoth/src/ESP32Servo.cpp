...
* registers.
*
* Different servos require different pulse widths to vary servo angle, but the
range is
* an approximately 500-2500 microsecond pulse every 20ms (50Hz). In general,
hobbyist servos
* sweep 180 degrees, so the lowest number in the published range for a
particular servo
* represents an angle of 0 degrees, the middle of the range represents 90
degrees, and the top
* of the range represents 180 degrees. So for example, if the range is 1000us to
2000us,
* 1000us would equal an angle of 0, 1500us would equal 90 degrees, and 2000us
would equal 180
* degrees. We vary pulse width (recall that the pulse period is already set to
20ms) as follows:
*
* The ESP32 PWM timers allow us to set the timer width (max 20 bits). Thus
* the timer "tick" length is (pulse_period/2**timer_width), and the equation for
pulse_high_width
...
*
* So, for example, if I want a 1500us pulse_high_width, I set pulse_period to
20ms (20000us)
* (this value is set in the ledcSetup call), and count (used in the ledcWrite
call) to
* 1500/(20000/65536), or 4924. This is the value we write to the timer in the
ledcWrite call.
* If we increase the timer_width, the timer_count values need to be adjusted.
*
* The servo signal pins connect to any available GPIO pins on the ESP32, but not
all pins are
* GPIO pins.
*
* The ESP32 is a 32 bit processor that includes FP support; this code reflects
that fact.
*/
...
Servo::Servo() {
  // initialize this channel with plausible values, except pin # (we set pin #
  // when attached)
  REFRESH_CPS = 50;
  this->ticks = DEFAULT_PULSE_WIDTH_TICKS;
  this->timer_width = DEFAULT_TIMER_WIDTH;
  this->pinNumber =
      -1;  // make it clear that we haven't attached a pin to this channel
...
int Servo::attach(int pin) {
  return (this->attach(pin, DEFAULT_uS_LOW, DEFAULT_uS_HIGH, DEFAULT_MIN_ANGLE,
                       DEFAULT_MAX_ANGLE));
}

int Servo::attach(int pin, int minPulse, int maxPulse, int minAngle,
                  int maxAngle) {
#ifdef ENFORCE_PINS
  ...
  if (pwm.hasPwm(pin)) {
    ...
    // we are attaching to a new or previously detached pin; we need to
  ...
  // Set up this channel
  // if you want anything other than default timer width, you must call
  // setTimerWidth() before attach
  pwm.attachPin(this->pinNumber, REFRESH_CPS,
                this->timer_width);  // GPIO pin assigned to channel
  // Serial.println("Attaching servo : "+String(pin)+" on PWM
...
void Servo::write(int value) {
  // treat values less than MIN_PULSE_WIDTH (500) as angles in degrees (valid
  ...
  if (value < this->minPulse) {
    ...
    value = map(value, this->minAngle, this->maxAngle, this->minPulse,
                this->maxPulse);
  }
...
int Servo::read() {  // return the value as degrees
  return (map(readMicroseconds() + 1, this->minPulse, this->maxPulse,
...
bool Servo::attached() { return (pwm.attached()); }
...
int Servo::readTimerWidth() { return (this->timer_width); }

int Servo::usToTicks(int usec) {
  return (int)((float)usec /
               ((float)REFRESH_USEC / (float)this->timer_width_ticks) *
               (((float)REFRESH_CPS) / 50.0));
}

int Servo::ticksToUs(int ticks) {
  return (int)((float)ticks *
               ((float)REFRESH_USEC / (float)this->timer_width_ticks) /
               (((float)REFRESH_CPS) / 50.0));
}
